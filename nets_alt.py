...
def residual_block(input, num_outputs, kernel_size, stride, scope):
    ...
    return net
def dense_block(input, num_layers, growth_rate, scope):
    with tf.compat.v1.variable_scope(scope):
        layers_concat = [input]
        x = input
        for i in range(num_layers):
            x = slim.batch_norm(x, activation_fn=tf.nn.relu, scope='bn_%d' % i)
            x = slim.conv2d(x, growth_rate, [3, 3], scope='conv_%d' % i)
            layers_concat.append(x)
            x = tf.concat(layers_concat, axis=-1)
    return x

def transition_layer(input, num_outputs, scope):
    with tf.compat.v1.variable_scope(scope):
        x = slim.batch_norm(input, activation_fn=tf.nn.relu, scope='bn')
        x = slim.conv2d(x, num_outputs, [1, 1], scope='conv')
        x = slim.avg_pool2d(x, [2, 2], stride=2, scope='pool')
    return x
    ...
    if struct ==0:
        ...
        with tf.compat.v1.variable_scope('depth_net') as sc:
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                            ...
                            activation_fn=tf.nn.relu,
                            outputs_collections=end_points_collection):
                x = slim.conv2d(tgt_image, 64, [7, 7], stride=2, scope='conv1')

                x = dense_block(x, 6, growth_rate, scope='dense_block1')
                x = transition_layer(x, 128, scope='trans1')

                x = dense_block(x, 12, growth_rate, scope='dense_block2')
                x = transition_layer(x, 256, scope='trans2')

                x = dense_block(x, 24, growth_rate, scope='dense_block3')
                x = transition_layer(x, 512, scope='trans3')

                x = dense_block(x, 16, growth_rate, scope='dense_block4')

                # Decoding path (unchanged, except for handling dense blocks)
                upcnv7 = slim.conv2d_transpose(x, 512, [3, 3], stride=2, scope='upcnv7')
                upcnv7 = resize_like(upcnv7, transition_layer(x, 512, scope='trans3'))
                i7_in = tf.concat([upcnv7, transition_layer(x, 512, scope='trans3')], axis=3)
                icnv7 = slim.conv2d(i7_in, 512, [3, 3], stride=1, scope='icnv7')

                upcnv6 = slim.conv2d_transpose(icnv7, 512, [3, 3], stride=2, scope='upcnv6')
                upcnv6 = resize_like(upcnv6, transition_layer(x, 256, scope='trans2'))
                i6_in = tf.concat([upcnv6, transition_layer(x, 256, scope='trans2')], axis=3)
                icnv6 = slim.conv2d(i6_in, 512, [3, 3], stride=1, scope='icnv6')

                upcnv5 = slim.conv2d_transpose(icnv6, 256, [3, 3], stride=2, scope='upcnv5')
                upcnv5 = resize_like(upcnv5, transition_layer(x, 128, scope='trans1'))
                i5_in = tf.concat([upcnv5, transition_layer(x, 128, scope='trans1')], axis=3)
                icnv5 = slim.conv2d(i5_in, 256, [3, 3], stride=1, scope='icnv5')

                upcnv4 = slim.conv2d_transpose(icnv5, 128, [3, 3], stride=2, scope='upcnv4')
                i4_in = tf.concat([upcnv4, dense_block(x, 6, growth_rate, scope='dense_block1')], axis=3)
                icnv4 = slim.conv2d(i4_in, 128, [3, 3], stride=1, scope='icnv4')
                disp4 = DISP_SCALING * slim.conv2d(icnv4, 1, [3, 3], stride=1,
                                                activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp4') + MIN_DISP
                disp4_up = tf.compat.v1.image.resize_bilinear(disp4, [H // 4, W // 4])
                ...
                upcnv3 = slim.conv2d_transpose(icnv4, 64, [3, 3], stride=2, scope='upcnv3')
                i3_in = tf.concat([upcnv3, transition_layer(x, 128, scope='trans1'), disp4_up], axis=3)
                icnv3 = slim.conv2d(i3_in, 64, [3, 3], stride=1, scope='icnv3')
                disp3 = DISP_SCALING * slim.conv2d(icnv3, 1, [3, 3], stride=1,
                                                activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp3') + MIN_DISP
                disp3_up = tf.compat.v1.image.resize_bilinear(disp3, [H // 2, W // 2])
                ...
                upcnv2 = slim.conv2d_transpose(icnv3, 32, [3, 3], stride=2, scope='upcnv2')
                i2_in = tf.concat([upcnv2, slim.conv2d(tgt_image, 64, [7, 7], stride=2, scope='conv1'), disp3_up], axis=3)
                icnv2 = slim.conv2d(i2_in, 32, [3, 3], stride=1, scope='icnv2')
                disp2 = DISP_SCALING * slim.conv2d(icnv2, 1, [3, 3], stride=1,
                                                activation_fn=tf.sigmoid, normalizer_fn=None, scope='disp2') + MIN_DISP
                disp2_up = tf.compat.v1.image.resize_bilinear(disp2, [H, W])
